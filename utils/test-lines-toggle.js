// 测试连线开关功能
console.log('=== 测试连线开关功能 ===');

// 模拟排盘组件的连线绘制逻辑
function simulateLinesDrawing(showLines) {
  console.log(`连线显示状态: ${showLines ? '显示' : '隐藏'}`);
  ...
  if (showLines) {
    ...
    for (const key of allLines) {
      ...
      const idx1 = LAYOUT_INDEX[line.palaces[0]];
      const idx2 = LAYOUT_INDEX[line.palaces[1]];
...
// 测试显示连线
simulateLinesDrawing(true);

console.log('\n--- 切换连线状态 ---');

// 测试隐藏连线
simulateLinesDrawing(false);
...
console.log('\n🎉 连线开关功能测试完成！');
console.log('连线开关应该能正确控制连线的显示和隐藏'); 