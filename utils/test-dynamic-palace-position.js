    ...
    if (palaceLayoutResult && palaceLayoutResult.success) {
      ...
      if (mingGong) {
        ...
        const branchToPositionMap = {
          '子': 0, '丑': 4, '寅': 8, '卯': 12, '辰': 13, '巳': 14,
          '午': 15, '未': 11, '申': 7, '酉': 3, '戌': 2, '亥': 1
        };
        
        const expectedPosition = branchToPositionMap[mingGong.branch];
        if (mingGong.layoutIndex === expectedPosition) {
          console.log(`✅ 命宫位置正确，地支 ${mingGong.branch} 应该在索引 ${expectedPosition}`);
        } else {
          console.log(`❌ 命宫位置错误，地支 ${mingGong.branch} 应该在索引 ${expectedPosition}，实际在索引 ${mingGong.layoutIndex}`);
        }
      } else {
        console.log(`❌ 未找到命宫`);
      }
